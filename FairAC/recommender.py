...
class DRRAgent:
    ...
    def recommend_item(self, action, recommended_items, k=False, items_ids=None):
        if items_ids == None:
        ...
        if k:
            item_indice = np.argsort(tf.transpose(tf.keras.backend.dot(items_ebs, action), perm=(1, 0)))[0][-k:]
            return items_ids[item_indice]
        else:
            item_idx = np.argmax(tf.keras.backend.dot(items_ebs, action))
            return items_ids[item_idx]

    def train(self, max_episode_num, k=False, load_model=False):

        self.actor.update_target_network()
        self.critic.update_target_network()

        if load_model:
            self.load_model("save_weights/actor_50000.h5",
                            "save_weights/critic_50000.h5")
            print('Completely load weights!')

        episodic_precision_history = []

        for episode in range(max_episode_num):
            # بازنشانی پاداش اپیزودیک
            episode_reward = 0
            correct_count = 0
            steps = 0
            q_loss = 0
            ...
            while not done:
                ...
                ## Item recommended
                recommended_item = self.recommend_item(action, self.env.recommended_items, k=k)

                # Calculate reward & observe new state (in env)
                ## Step
                next_items_ids, reward, done, _ = self.env.step(recommended_item, k=k)
                if k:
                ...
                if self.buffer.crt_idx > 1 or self.buffer.is_full:
                    ...
                    min_qs = tf.raw_ops.Min(input=tf.concat([target_qs, qs], axis=1), axis=1,
                ...
                if done:
                    ...
                    # print(
                    #     f'{episode}/{max_episode_num}, precision : {precision:2}%, total_reward:{episode_reward}, q_loss : {q_loss / steps}')
                    # if self.use_wandb:
